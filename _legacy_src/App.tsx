...
import { BarChart3, User, LogOut, PlusCircle, Home, ListChecks, ChevronDown, Users } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  if (!currentUser) {
    const isSignin = currentHash === '#/signin';
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-gray-900">
          ...
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-4">
              ...
              <nav className="hidden lg:flex items-center gap-1 ml-6 border-l border-gray-100 pl-6">
                ...
                {currentUser.role !== UserRole.SYS && (
                  <button onClick={() => navigateTo('/plan')} className={`flex items-center px-4 py-2 rounded-xl text-xs font-bold transition-all ${[Tab.PLAN, Tab.REPORT, Tab.KPI].includes(activeTab) ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100'}`}>
                    <PlusCircle size={16} className="mr-2" /> Báo cáo / KPI
                  </button>
                )}

                {(isManager || isAdmin) && (
                  <button onClick={() => navigateTo('/approvals')} className={`flex items-center px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === Tab.APPROVALS ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100'}`}>
                    <ListChecks size={16} className="mr-2" /> Duyệt báo cáo
                  </button>
                )}

                {isAdmin && (
                  <button onClick={() => navigateTo('/admin')} className={`flex items-center px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === Tab.USER_MGMT ? 'bg-red-600 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100'}`}>
                    <Users size={16} className="mr-2" /> Cấu hình nhân sự
                  </button>
                )}
              </nav>
            </div>
            ...
            <div className="flex items-center gap-3">
               <div className="relative">
                 <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-2 hover:bg-gray-50 p-1.5 rounded-xl transition-colors">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-6">
        <div className="bg-white rounded-3xl shadow-sm border border-gray-200 min-h-[75vh] flex flex-col overflow-hidden relative">
